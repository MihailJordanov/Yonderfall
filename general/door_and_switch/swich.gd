...
const DOOR_SWITCH_AUDIO = preload( "uid://dlj42sawr3oqb" )
...
func _on_player_entered( _n : Node2D ) -> void:
	...
	Messages.player_interact.connect( _on_player_interacted )
	pass

func _on_player_exited( _n : Node2D ) -> void:
	Messages.input_hint_changed.emit( "" )
	Messages.player_interact.disconnect( _on_player_interacted )
	pass
